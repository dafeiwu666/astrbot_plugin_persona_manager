...
class NicknameSync:
    ...
    def load_config(self, config: dict | None) -> None:
        ...
        if self.nickname_sync_mode not in {"profile", "group_card", "hybrid"}:
            logger.warning(
                f"卡片社区化人设管理 昵称同步模式 {self.nickname_sync_mode} 无效，将使用默认值 group_card"
            )
            self.nickname_sync_mode = "group_card"
    ...
    def format_nickname(self, persona_name: str) -> str:
        ...
        try:
            ...
            logger.warning(f"卡片社区化人设管理 昵称模板解析失败：{exc}，使用人设名")
        ...
        *,
        ...
        if not persona_name or persona_name == EMPTY_PERSONA_NAME:
            ...
            if external_persona_name:
                ...
                # 外部人设名也为空，不同步
                logger.debug("卡片社区化人设管理 插件内外人设均为空，跳过昵称同步")
                return
        ...
        if not force and self._last_synced_persona.get(bot_key) == display_name:
            logger.debug(f"卡片社区化人设管理 人设 {display_name} 已同步过，跳过 (Key: {bot_key})")
            return
        ...
        if nickname_applied:
            ...
            logger.info(f"卡片社区化人设管理 已同步昵称为 {display_name}")
    ...
    async def _sync_qq_profile(
        ...
        if hasattr(bot, "set_qq_profile"):
            try:
                ...
                logger.debug(f"卡片社区化人设管理 已同步 QQ 昵称为 {nickname}")
                return True
            except Exception as exc:
                logger.error(f"卡片社区化人设管理 同步 QQ 昵称失败：{exc}")
        else:
            logger.warning(
                "卡片社区化人设管理 当前适配器未实现 set_qq_profile 接口，跳过 QQ 昵称同步。"
            )
        return False
    ...
    async def _sync_group_card(self, event: AstrMessageEvent, card: str) -> bool:
        ...
        if hasattr(bot, "call_action"):
            try:
                ...
                logger.debug(f"卡片社区化人设管理 已同步群名片为 {card} (群 {group_id})")
                return True
            except Exception as exc:
                logger.error(f"卡片社区化人设管理 同步群名片失败：{exc}")
        else:
            logger.warning(
                "卡片社区化人设管理 当前适配器未实现 call_action 接口，跳过群名片同步。"
            )
        return False
