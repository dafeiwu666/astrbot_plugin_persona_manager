## 📖 简介
 ...
 本插件（**卡片社区化人设管理**）为 AstrBot 提供“卡片注入 / 人设管理”能力：你可以创建多张卡片（人设/设定），并在对话时一键切换；插件会在 LLM 请求前把当前卡片内容注入到 system prompt，从而让主 AI 以指定人设进行回复。

同时支持：
- **会话式创建/修改卡片**（引导式输入简介、标签、设定内容）
- **群聊/私聊分别维护当前卡片**（同一用户在不同群可使用不同卡片）
- **关键词触发临时人设**（匹配关键词后仅注入对应提示词，不注入当前卡片与前后缀）
- **CozyNook 社区卡片小屋**（浏览帖子、导入为本地卡片、导出附件；也可在网页端上传/发布人设并分享给大家）
- **可选：统一外部 persona、自动重置聊天记录、昵称/群名片同步**

## 必做
- **空人设**：在 Astr 自带的人设管理里先建一个空人设（内容为空），把该人设 ID 填到配置项 `external_persona_id`，即可用本插件全权代理人设注入。

## ✨ 特性

- **🧩 卡片管理（卡包）**：创建、查看、列表、标签搜索、切换、删除、修改
- **🧷 注入包装器**：可为卡片内容套用统一的前置/后置提示词（`default_prefix` / `default_suffix`）
- **🧼 正则清洗（可选）**：可对“注入的卡片内容”在注入前执行正则清洗（支持使用配置默认或每卡片自定义）
- **🧠 LLM Hook 注入**：在 `on_llm_request` 阶段追加到 `system_prompt`，避免覆盖其它插件/系统预设
- **🧯 安全防循环**：忽略机器人自身消息与空回流事件，避免会话状态机自触发
- **🧾 每日次数限制**：支持群聊/私聊按日限额（可按白名单放行）
- **🏠 CozyNook 卡片小屋**：支持 `ula-xxxxxxxxxxxxxxxx`（16位）密码获取/导入/导出

## 📦 安装

1. 确保你已经安装了 [AstrBot](https://github.com/Soulter/AstrBot)。
2. 将本插件文件夹放入 AstrBot 的 `data/plugins/` 目录下。
3. 安装依赖：CozyNook（卡片小屋）联网功能依赖 `aiohttp`。
    ```bash
    pip install aiohttp
    ```
...
| `default_prefix` / `default_suffix` | 卡片注入前后缀 | 仅当卡片启用包装器时生效 |
| `default_clean_regex` | 默认正则文本清洗表达式 | 对卡片内容执行 `re.sub(pattern, '', text)`；留空表示不启用默认清洗 |
| `session_timeout_sec` | 会话式创建/编辑超时（秒） | |
| `external_persona_id` | 统一外部 persona ID | 配置后可实现“对话内外部 persona 统一” |
| `auto_reset_on_switch` | 切换卡片时自动重置聊天记录 | |
| `sync_nickname_on_switch` | 切换卡片时同步昵称/群名片 | 仅部分平台支持（如 aiocqhttp） |
...
| `cozynook_channel_cards_pick` | 卡片小屋频道抽卡数量 | `1-30`，默认 15 |
...
| 指令 | 说明 |
| :--- | :--- |
| `/创建卡片 名称` | 会话式创建卡片（按提示输入简介/标签/设定内容） |
| `/卡包` | 列出你的所有卡片（合并转发） |
| `/查找卡片` | 分段查找：先选 `/卡包`（按标签）或 `/小屋`（按关键词搜帖） |
| `/查看卡片 名称` | 查看卡片详情（含标签与简介） |
| `/切换卡片 名称` | 切换当前会话（群/私聊维度）的卡片 |
| `/休息模式` | 清空当前会话卡片（不注入任何卡片） |
| `/当前卡片` | 查看当前会话正在使用的卡片 |
| `/修改卡片 卡片名` | 会话式修改简介/标签/设定内容 |
| `/删除卡片 名称` | 删除卡片 |

### 🏠 CozyNook 卡片小屋

CozyNook 是一个偏“社区分享”的卡片小屋：你可以在网页端发布/上传人设，生成 `ula-xxxxxxxxxxxxxxxx`（16位）密码；群友通过本插件输入 ULA 即可一键拉取并导入。

同时，`/卡片小屋` 会从指定分享码频道中随机抽取卡片，并直接给出“标题 + 密码”。

| 指令 | 说明 |
| :--- | :--- |
| `/卡片小屋` | 输出频道分区列表 + 随机抽取卡片（标题 + 密码） |
| `/卡片小屋 分区名` | 仅从该分区随机抽取卡片 |
| `/获取卡片 ula-xxxxxxxxxxxxxxxx` | 打开指定卡片/帖子（可选择导入或导出） |
| `/导入卡片 ula-xxxxxxxxxxxxxxxx` | 直接导入为本地卡片（导入后会切回休息模式） |
| `/导出卡片 ula-xxxxxxxxxxxxxxxx` | 直接进入导出流程（选择附件序号下载） |
...
- 卡片与状态持久化存放在 AstrBot 数据目录下：`astrbot_plugin_persona_manager/store.json`
...
## 关于社区
  ...
  关于API部分我也做了对应的后端接入API，以下是API文档：
    ...
    **硬编码地址**：API 基地址硬编码在 [src/pm_commands_cozynook.py](src/pm_commands_cozynook.py) 中（根目录 [pm_commands_cozynook.py](pm_commands_cozynook.py) 仅做兼容转发），固定为 `https://c0zynook.com/api`。
    ...
    1.  **卡片/帖子拉取**：指令 `/获取卡片 ula-xxxx`，以“合并转发”形式展示帖子详情（标题、简介、正文）。
      2.  **社区活跃展示**：在详情中附带最新评论内容。
      3.  **附件管理**：解析 Files 字段，支持按序号下载并发送图片/文件（含缓存与自动清理）。
    4.  **一键导入**：将在线帖子内容转换为本地卡片（导入后自动切入休息模式以防冲突）。